...
def create_visualization(filename):
    # Using the res dictionary directly as skills
    res = Utils.res

    # Create the sizes and names lists from res
    names = list(res.keys())
    ...
    plt.pie(sizes, labels=names, colors=colors, startangle=90, counterclock=False)
    plt.axis('equal')  # Equal aspect ratio ensures that pie is drawn as a circle.
    plt.savefig("static/images/sunburst.png", format="png")

# Call the function to create the visualization
create_visualization(filename="cv.pdf")